...
const getById = (request, response)=>{
    ...
    const tarefaSelected = tarefasModels.find(tarefa => tarefa.id == id)
    response.status(200).send(tarefaSelected)
}
...
const getTarefaNaoConcluida = (request, response)=>{
    const tarefaNconcluida = tarefasModels.filter(tarefa => tarefa.concluido == false)
    response.status(200).send(tarefaNconcluida)
}
